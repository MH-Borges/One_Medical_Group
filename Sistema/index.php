...
if (!empty($chave)) {
    ...
    $dados = $res->fetchAll(PDO::FETCH_ASSOC);
    $senha_crip = md5('OneMedical312@@');
    $data = date('Y/m/d H:i:s');
    
    if(@count($dados) == 0){
        $res = $pdo->query("INSERT into medicos (nivel, status_perfil, email, senha, senha_Crip, data_Registro) values ('adm', 'ativo', 'administrador@onemedicalgroup.com.br', 'OneMedical312@@', '$senha_crip', '$data' )");
    }
...
<head>
    ...
    <?php 
        if(!$Recup_True){
            echo "<title> One Medical Group | Login </title>";
        }
        else{
            echo "<title> One Medical Group | Recuperação de senha </title>";
        }
    ?>
                ...
                <form id="Form_ModalrecupSenha" method="post" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="BlockBox">
                            ...
                            <p class="lengthInput EmailResetSenhaInput"> 415 </p>
                        </div>
                    </div>
